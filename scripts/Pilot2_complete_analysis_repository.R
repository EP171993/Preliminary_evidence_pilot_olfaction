...
# Olfactory Pod (P) versus No Olfactory Pod (A), N = 47
# ============================================================================
#
# INPUT
#   pilot_2_data_.xlsx
...
# ----------------------------------------------------------------------------
# 6. NUTRITIONAL PROPERTIES - REPEATED-MEASURES ART
# ----------------------------------------------------------------------------
# Full repeated-measures error structure, matching the taste model (Section 8).
# Interaction error stratum: 2 x 46 = 92 df.
nutrition_art <- ARTool::art(
  nutritional_rating ~ nutritional_property * condition +
    Error(subject / (nutritional_property * condition)),
  data = nutrition_long
)
...
for (effect_name in c("factor_a", "factor_b", "interaction")) {
  ...
  model <- aov(
    as.formula(paste0(
      rank_name,
      " ~ nutritional_property * condition + ",
      "Error(subject / (nutritional_property * condition))"
    )),
    data = nutrition_long
  )

  nutrition_art_tables[[effect_name]] <- tidy_aovlist(model) %>%
    mutate(alignment = effect_name)
}

nutrition_manual_art <- bind_rows(nutrition_art_tables)

# Interaction comes from the interaction-aligned model.
nutrition_interaction_row <- nutrition_manual_art %>%
  filter(
    alignment == "interaction",
    str_trim(term) %in% c("nutritional_property:condition",
                          "condition:nutritional_property")
  )

assert_true(nrow(nutrition_interaction_row) == 1,
            "Nutrition ART interaction row found")
assert_close(nutrition_interaction_row$F.value, 47.487, .05,
...
factorial_results_table <- bind_rows(
  ...
  nutrition_contrasts %>%
    transmute(
      ...
      term = "No Olfactory Pod versus Olfactory Pod",
...
# ----------------------------------------------------------------------------
# 10. ESTIMATED INGESTION AMOUNT: POD VS NO OLFACTORY POD
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 10b. PAIRED T-TEST: POD VS NO OLFACTORY POD
# ----------------------------------------------------------------------------
...
ingestion_results <- tibble(
  ...
  mean_no_olfactory_pod = mean_A,
  sd_no_olfactory_pod = sd_A,
...
cat("\n====================================================\n")
cat("ESTIMATED INGESTION: POD VS NO OLFACTORY POD\n")
cat("====================================================\n")
...
cat("\n====================================================\n")
cat("NORMALITY OF ABSOLUTE-ERROR DIFFERENCES: POD - NO POD\n")
cat("====================================================\n")
...
absolute_error_results <- tibble(
  ...
  mean_absolute_error_no_pod = mean(
    absolute_error_data$abs_error_A
  ),
  
  sd_absolute_error_no_pod = sd(
    absolute_error_data$abs_error_A
  ),
  
  median_absolute_error_no_pod = median(
    absolute_error_data$abs_error_A
  ),
...
cat("\n====================================================\n")
cat("ABSOLUTE ESTIMATION ERROR: POD VS NO OLFACTORY POD\n")
cat("====================================================\n")
...
assert_close(
  ...
  80.8617,
  .01,
  "Mean absolute error in No Olfactory Pod"
)
...
assert_true(
  ...
  "Pod is rated as more intense than No Olfactory Pod"
)
...
single_outcome_table <- bind_rows(
  ingestion_results %>%
    transmute(
      ...
      no_olfactory_pod = paste0(
        sprintf("%.2f", mean_no_olfactory_pod),
        " (", sprintf("%.2f", sd_no_olfactory_pod), ")"
      ),
  ...
  intensity_pleasantness_results %>%
    transmute(
      ...
      no_olfactory_pod = sprintf("%.2f", median_no_olfactory_pod),
...
open_paired <- condition_wide %>%
  mutate(
    ...
    pattern = case_when(
      binary_code_A == 0 & binary_code_P == 1 ~ "No Olfactory Pod plain water -> Olfactory Pod attributed beverage identity",
      binary_code_A == 1 & binary_code_P == 1 ~ "Attributed beverage identity in both conditions",
      binary_code_A == 1 & binary_code_P == 0 ~ "No Olfactory Pod attributed beverage identity -> Olfactory Pod plain water",
...
assert_true(water_plain_n == 31,
            "No Olfactory Pod plain-water identifications = 31/47")
...
assert_true(expected_shift_n == 30,
            "Expected No-Pod-to-Pod shifts = 30/47")
assert_true(reverse_shift_n == 1,
            "Reverse Pod-to-No-Pod shift = 1/47")
...
assert_close(100 * water_plain_n / 47, 66.0, .1,
             "No Olfactory Pod identified as plain water (%)")
...
open_matrix_records <- as.data.frame(open_matrix) %>%
  transmute(
    ...
    row_category = if_else(
      A == "0",
      "No Olfactory Pod: plain water",
      "No Olfactory Pod: attributed beverage identity"
    ),
...
verification_summary <- tibble(
  claim = c(
    ...
    "No Olfactory Pod plain-water identifications",
  ...
  pass = c(
    ...
    nrow(dat) == 94,
    abs(nutrition_interaction_row$F.value - 47.487) <= .05,
...
# 15. PLOTS
# ----------------------------------------------------------------------------
condition_labels <- c("A" = "No Olfactory Pod", "P" = "Olfactory Pod")
...
p_ingestion <- condition_wide %>%
  select(subject,
         `No Olfactory Pod` = ingested_ml_estimate_A,
         `Olfactory Pod`    = ingested_ml_estimate_P) %>%
  pivot_longer(c(`No Olfactory Pod`, `Olfactory Pod`),
               names_to = "condition_plot", values_to = "estimated_ml") %>%
  mutate(condition_plot = factor(condition_plot,
                                 levels = c("No Olfactory Pod", "Olfactory Pod"))) %>%
  ...
  scale_color_manual(values = c("No Olfactory Pod" = unname(condition_colors["A"]),
                                "Olfactory Pod"    = unname(condition_colors["P"]))) +
...
open_plot_data <- open_by_condition %>%
  mutate(
    condition = factor(condition, levels = c("A", "P"), labels = c("No Olfactory Pod", "Olfactory Pod")),
...
main_figure <- (p_nutrition / p_taste) / (p_ingestion | p_ip | p_open) +
  patchwork::plot_annotation(
    title = "Pilot 2: Comparison of the Olfactory Pod versus No Olfactory Pod contrast",
...
p_reg_sweet <- ggplot(condition_wide, aes(diff_sweet, nutritional_composite_z)) +
  ...
  labs(title = "Sweetness and nutritional attribution",
       ...
       x = "Pod - No Pod sweetness", y = "Nutritional composite (z)") +
...
p_reg_bitter <- ggplot(condition_wide, aes(diff_bitter, nutritional_composite_z)) +
  ...
  labs(title = "Bitterness and nutritional attribution",
       ...
       x = "Pod - No Pod bitterness", y = "Nutritional composite (z)") +
  base_theme + theme(legend.position = "none")
...
p_reg_fat_bitter <- ggplot(condition_wide, aes(diff_bitter, diff_fat)) +
  ...
  labs(title = "Selective bitterness-fat association",
       ...
       x = "Pod - No Pod bitterness", y = "Pod - No Pod perceived fat") +
  base_theme + theme(legend.position = "none")
